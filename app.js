...
async function handleAuthStateChanged(user) {
  ...
  if (user) {
    try {
      ...
      if (!user.isAnonymous && hasGuestData) {
        ...
        if (shouldMerge) {
          ...
          // addXP now targets only non-anonymous accounts, so this transfers
          // local guest XP into both the account's local bucket and its next
          // batched Firestore increment.
 ...
 * `injectRegister` is disabled in vite.config.js because the plugin would
 * inject an INLINE registration script, which our CSP (`script-src 'self'`)
 * blocks — so registration happens here from same-origin app code.
 * Without this, dist/sw.js + workbox-*.js are deployed but never activate,
 * so the PWA has no offline capability at all.
 */
function registerServiceWorker() {
  if (!import.meta.env.PROD || !('serviceWorker' in navigator)) return;
  window.addEventListener('load', () => {
    // BASE_URL is './' for this app, so this resolves next to index.html.
...
function setupEventListeners() {
  ...
  document.getElementById('import-input')?.addEventListener('change', async (e) => {
    ...
    if (result?.success) {
      ...
      if (importedSettings.theme) ThemeManager.apply(importedSettings.theme);
      if (importedSettings.language) {
        ...
        storageSet('pixelWordHunter_language', importedSettings.language);
        await I18nManager.setLanguage(importedSettings.language);
        store.setState({
          uiLanguage: importedSettings.language,
          translationLanguage: importedSettings.language === 'ko' ? 'ko' : 'ru'
        });
      }
      if (typeof importedSettings.audio === 'boolean') {
        storageSet('pixelWordHunter_muted', String(!importedSettings.audio));
        store.setState({ audioEnabled: importedSettings.audio });
      }
      refreshCategoryButtons();
      ...
      // remain local-only.
      try {
        await saveProgress(firebaseDb, doc, setDoc, serverTimestamp);
